...
import { 
  ...
  ResponsiveContainer, Dot, ReferenceLine
} from 'recharts';
import { BitcoinPrice, NewsEvent } from '../types';
import { fetchNewsForPrice } from '../services/api';
import { format } from 'date-fns';
import { ChevronDown, ChevronUp, TrendingUp, AlertCircle, ExternalLink } from 'lucide-react';
...
const CustomDot = (props: any) => {
  ...
  return (
    <Dot
      ...
      cy={cy}
      r={3.5}
      className={`timeline-dot ${hasNews ? 'timeline-dot-with-news' : 'timeline-dot-without-news'}`}
    />
  );
};
...
const NewsItem = ({ news }: { news: NewsEvent }) => {
  return (
    <div className={`news-item-${news.sentiment} p-4 mb-4 bg-white dark:bg-gray-800 rounded-lg shadow`}>
      ...
      <div className="flex justify-between items-center text-xs mt-2">
        <span className="text-gray-500 dark:text-gray-400">Source: {news.source}</span>
        <a 
          href={news.url} 
          target="_blank" 
      ...
      <div className="h-80 md:h-96">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart
            ...
            onClick={(e) => e?.activePayload && handleDotClick(e.activePayload[0].payload)}
          >
